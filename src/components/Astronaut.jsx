...
export function Astronaut(props) {
  ...
  useEffect(()=>{
    if(animations.length>0){
      ...
      console.log(actions[animations[0].name])
    }
  },[actions,animations]);
  // first position will be top :5
  const yPosition = useMotionValue(5);
  // make animation when element move
  const ySpring = useSpring(yPosition,{damping:30});
  useEffect(()=>{
    // here will the element stop (from 5 at the top to the element current position)
    ySpring.set(-1)
  },[ySpring]);
  // smooth effect
  useFrame(()=>{
  ...
  return (
    <group ref={group} {...props} dispose={null}  rotation={[-Math.PI / 2, -0.2, 2.0]} scale={props.scale||0.3}position={props.position||[1.3,-1,0]}>
